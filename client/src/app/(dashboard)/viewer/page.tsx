  ...
  return (
    <div className="space-y-16">
      <div className="flex flex-col gap-4">
        <h1 className="text-6xl font-semibold tracking-tighter text-black">Financial Review</h1>
        <p className="text-xl text-slate-500">Summary of activity across all tracked categories.</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-12 md:grid-cols-3">
        <StatCard label="Balance" value={`$${data?.summary.netBalance.toLocaleString()}`} isPrimary />
        <StatCard label="Income" value={`$${data?.summary.totalIncome.toLocaleString()}`} />
        <StatCard label="Expenses" value={`$${data?.summary.totalExpenses.toLocaleString()}`} />
      </div>

      <div className="mt-24 space-y-10">
        <h3 className="text-2xl font-semibold text-black uppercase tracking-tight">Recent Activity</h3>
